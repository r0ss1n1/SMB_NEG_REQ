#include <stdio.h>
#include <winsock2.h>
#include <ws2tcpip.h>
#include <synchapi.h>


#pragma comment(lib, "ws2_32.lib")

int main(void) {

	/* NTLMSSP_AUTH fails / SessionID needs to be kept on both sides */

	/* Charles Thomas Wallace Truscott @r0ss1n1 github.com/r0ss1n1 */

	/* Shout out to former DG ASIO/ASIS David Taylor Irvine AO */

	WSADATA wsaData;
	int result = WSAStartup(MAKEWORD(2, 2), &wsaData);
	char PACKET_1[76];
	memcpy(PACKET_1, "", 73);
	char PACKET_2[179];
	memcpy(PACKET_2, "", 178);
/*	char PACKET_3[76];
	memcpy(PACKET_3,
	char PACKET_4[76];
	memcpy(PACKET_4, 
	char PACKET_1[76];
	memcpy(PACKET_5, 
*/

/*
	int PACKET_1[74];
	memset(PACKET_1, , 1);
	"", 1);
	"";
	"";
	"";
*/


	SOCKET SMB_CONNECTION;
	SMB_CONNECTION = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
	struct sockaddr_in SMB_DEETS;
	for(int i = 0; i < 73; ++i){
		printf("%x", PACKET_1[i]);
	}
	printf("\n");
	printf("\n\n");
	SMB_DEETS.sin_family = AF_INET;
	SMB_DEETS.sin_addr.s_addr = inet_addr("127.0.0.1");
	SMB_DEETS.sin_port = htons(445);
	result = connect(SMB_CONNECTION, (SOCKADDR *) &SMB_DEETS, sizeof(SMB_DEETS));
	if(result == SOCKET_ERROR) {
		printf("socket error: %ld", WSAGetLastError());
		closesocket(SMB_CONNECTION);
	}
	Sleep(3000);
	result = send(SMB_CONNECTION, PACKET_1, 73, 0);
	printf("sent %d bytes\n", result);
	Sleep(3000);
	result = shutdown(SMB_CONNECTION, SD_SEND);
	Sleep(3000);
	char PACKET_RECV[512];
	result = recv(SMB_CONNECTION,(char *) PACKET_RECV, 512, 0);
	Sleep(6000);
	printf("%d\n", WSAGetLastError());

	for(int i = 0; i < 512; ++i){
		printf("%x", PACKET_RECV[i]);
	}
	printf("\n");
	for(int i = 0; i < 178; ++i){
		printf("%x", PACKET_2[i]);
	}
	result = send(SMB_CONNECTION, PACKET_2, 178, 0);
	result = shutdown(SMB_CONNECTION, SD_SEND);
	char PACKET_RECV_2[512];
	result = recv(SMB_CONNECTION,(char *) PACKET_RECV_2, 512, 0);
	Sleep(6000);
	printf("%d\n", WSAGetLastError());

	for(int i = 0; i < 512; ++i){
		printf("%x", PACKET_RECV_2[i]);
	}
	printf("\n");
	closesocket(SMB_CONNECTION);

}